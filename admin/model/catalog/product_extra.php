...
class ModelCatalogProductExtra extends Model {
	...
	public function getProducts($data = array(), $language = null) {
		...
		if ($data) {
			...
			if (isset($data['filter_meta_title']) && !is_null($data['filter_meta_title'])) {
				$sql .= " AND LCASE(pd.meta_title) LIKE '%" . $this->db->escape(strtolower($data['filter_meta_title'])) . "%'";
			}
			...
			if (isset($data['filter_upc']) && !is_null($data['filter_upc'])) {
				$sql .= " AND LCASE(p.upc) LIKE '%" . $this->db->escape(strtolower($data['filter_upc'])) . "%'";
			}
			
			if (isset($data['filter_location']) && !is_null($data['filter_location'])) {
				$sql .= " AND LCASE(p.location) LIKE '%" . $this->db->escape(strtolower($data['filter_location'])) . "%'";
			}
			if (isset($data['filter_date_available']) && !is_null($data['filter_date_available'])) {
				$sql .= " AND LCASE(p.date_available) LIKE '%" . $this->db->escape(strtolower($data['filter_date_available'])) . "%'";
			}
	...
	public function getAlias($product_id){
		$row = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id. "'");
	...
	public function changeSeo($product_id, $keyword){
