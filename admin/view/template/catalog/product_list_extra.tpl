    ...
    <div class="heading">
      <div class="row">
        ...
        <div class="col-md-8 text-right">
          <div class="btn-group language-selector">
            ...
            if(isset($languages)){
              foreach($languages as $lang=>$language){
                ...
                $options[] = '<li><a href="#" data-code="'.$language['code'].'" data-id="'.$language['language_id'].'">'.$language['name'].'</a></li>';
              }
            }
            ?>
            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
              Language: <span id="selected-language"><?php echo (isset($selected_lang) ? $selected_lang : ''); ?></span> <span class="caret"></span>
            </button>
          ...
          <a onclick="copyDefaultProduct(); return false;" class="btn btn-info copy-default-product" href="<?php echo $copy; ?>" data-toggle="tooltip" title="<?php echo $copy_default_product;?>"><i class="glyphicon glyphicon-file"></i></a>
          <a onclick="editDefaultProduct()" class="btn btn-info default-product" href="index.php?route=catalog/product/edit&token=<?php echo $token; ?>&product_id=-1" data-toggle="tooltip" title="<?php echo $edit_default_product;?>"><i class="glyphicon glyphicon-gift"></i></a>
          <a class="btn btn-warning columns-button" title="<?php echo $column_switch; ?>" data-toggle="tooltip"><i class="fa fa-check-square-o"></i></a>
          <!-- <a class="btn btn-default settings-button" title="<?php echo $settings_switch; ?>" data-toggle="tooltip"><i class="glyphicon glyphicon-cog"></i> </a> -->
...
<!--Length class-->
              ...
              <td class="text-center"><a onclick="filter();" class="btn btn-default button"><span><?php echo $button_filter; ?></span></a></td>
            </tr>
...
<!-- product_descriptions-->
              ...
              <td class="center product-image image-column <?php echo ((isset($_COOKIE['image-column']) && $_COOKIE['image-column'] == 1)?'':'hide-column');?>" rel="<?php echo $link;?>&type=change_image&product_id=<?php echo $product['product_id'];?>">
                  ...
                  <a href="" id="thumb-image-<?php echo $product['product_id'];?>" data-toggle="main-image" class="img-thumbnail">
                    ...
                    <input type="hidden" name="image" value="<?php echo $product['image']; ?>" data-id="<?php echo $product['product_id'];?>" id="image-<?php echo $product['product_id'];?>" rel="index.php?route=catalog/product_extra&type=change_image&token=<?php echo $token; ?>"/>
...
<?php if(!isset($ajaxed)){?>
  ...
  function filter() {
          ...
          var filter_id = $('input[name=\'filter_id\']').prop('value');
          if (filter_id) {
          ...
          var filter_meta_title = $('input[name=\'filter_meta_title\']').prop('value');
          ...
          var filter_weight = $('input[name=\'filter_weight\']').prop('value');
          
          if (filter_weight) {
                  url += '&filter_weight=' + encodeURIComponent(filter_weight);
          }
          
          var filter_sku = $('input[name=\'filter_sku\']').prop('value');
          
          if (filter_sku) {
                  url += '&filter_sku=' + encodeURIComponent(filter_sku);
          }

          var filter_upc = $('input[name=\'filter_upc\']').prop('value');
          
          if (filter_upc) {
                  url += '&filter_upc=' + encodeURIComponent(filter_upc);
          }
          var filter_ean = $('input[name=\'filter_ean\']').prop('value');
          
          
          var filter_location = $('input[name=\'filter_location\']').prop('value');
          
          if (filter_location) {
                  url += '&filter_location=' + encodeURIComponent(filter_location);
          }
          var filter_date_available = $('input[name=\'filter_date_available\']').prop('value');
          ...
          var filter_subtract = $('select[name=\'filter_subtract\']').prop('value');
          ...
          var filter_length_class = $('select[name=\'filter_length_class\']').val();
          ...
          var filter_weight_class = $('select[name=\'filter_weight_class\']').val();
          ...
          var filter_sort_order = $('input[name=\'filter_sort_order\']').prop('value');
  ...
  $(document).on('change', 'td.discount-column input.discount-price-field', function(e){
  ...
  $(document).on('change', 'td.special-column input.special-price-field', function(e){
    var td = $(this).parents('td');
    var tds = $(this).parents('tr').children('td');
    var product_id = $(tds[0]).find('input').val();
    var trigger = $(this);
  ...
  var link = 'index.php?route=catalog/product_extra&token=<?php echo $token; ?>';
  var no_image = '<?php echo $no_image; ?>';
  var text_product_manager = '<?php echo $text_product_manager; ?>';
  $(document).ready(function($) {
    $(document).on('keydown', 'tr.filter input', function(e) {
          if (e.keyCode == 13) {
                  filter();
          }
    });
    $(document).on('focusout', 'tr.filter input', function(e) {
          filter();
    });
    
    $(document).on('keydown', '.product-row :input', function(e) {
            if (e.keyCode == 13) {
              $(this).trigger('blur');
              $(this).focus();
              return false;
    ...
    $(document).on('click', 'button.discount-save', function(e){
        var trigger = $(this);
        e.preventDefault();
    ...
    $(document).on('click', 'a.special_link, a.discount_link', function(e){
      e.preventDefault();
      var trigger = $(this);
    ...
    $(document).on('click', '.delete-button', function(e){
      ...
      $('#form-product-extra').attr('action', 'index.php?route=catalog/product_extra/delete&token=<?php echo $token; ?>');
