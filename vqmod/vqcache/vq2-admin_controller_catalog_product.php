...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['keyword'])) {
			...
			$data['keyword'] = $this->model_catalog_product->getKeyWords($this->request->get['product_id']);
		} else {
			$data['keyword'] = array();
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen(current($this->request->post['keyword'])) > 0) {
			$this->load->model('catalog/url_alias');

			$url_alias_info = $this->model_catalog_url_alias->getUrlAlias(current($this->request->post['keyword']));
